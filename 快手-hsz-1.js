...
threads.start(function() {
    ...
    while (true) {
      ...
      if(sj1>10*60*1000){
        log('jieshu')
        url = "http://wxpusher.zjiecode.com/api/send/message/?appToken=AT_GRDG2ZQu57APwWcSxZpavjK3qNl9tCfO&content=hsz1-chucuole&uid=UID_JNz25wHNb8ExLeaoWTMY98BFuIcL&url=http%3a%2f%2fwxpusher.zjiecode.com"//写入
...
url = "http://61.191.56.132:20177/jk/post-cha.php?fname=HSZ1-KSJ"//编号
...
for (i = 0; i < 1111; i++) {//232 322//206 207-211 275 289   327
  ...
  if(!device.isScreenOn())
    ...
    pxx()
    cx2()
   data = {fname: "HSZ1-KSJ", age: a+1,c:Date()}
...
function cx() {
    launchApp('CX文件管理器')
    sleep(4001)
    toastLog(textContains("a01/wa1").findOne().parent().click())
    sleep(4001)
